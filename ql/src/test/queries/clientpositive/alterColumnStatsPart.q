...
desc formatted p partition (c=100) a;
...
analyze table p partition(c=4) compute statistics for columns a;

desc formatted p partition(c=4);

explain select max(a) from p where c=4;

explain select max(b) from p where c=4;

set hive.stats.column.autogather=true;

insert into p partition (c) values (20,30,4);

desc formatted p partition(c=4);

explain select min(a) from p where c=4;

explain select min(b) from p where c=4;

select min(a) from p where c=4;

select min(b) from p where c=4;
